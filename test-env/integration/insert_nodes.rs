...
use std::fs;
fn main(){
    let storage = std::env::var("STORAGE").unwrap();
    let a = fs::read_to_string("a.rs").unwrap().into_bytes();
    let b = fs::read_to_string("b.rs").unwrap().into_bytes();
    ...
    let gs = GraphStore::open(storage+"/graph-db").unwrap();
    gs.insert_node(&node_a).unwrap();
    gs.store_content(&id_a, &a).unwrap();
    gs.insert_node(&node_b).unwrap();
    gs.store_content(&id_b, &b).unwrap();
    gs.add_dependency(&id_b, &id_a, EdgeType::Imports).unwrap();
