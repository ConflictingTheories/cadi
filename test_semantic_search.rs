...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let registry = RegistryDatabase::new(db, Some(embedding_manager)).await?;
...
export default app;"#,
    ...
    for (chunk_id, content, language, description, concepts) in chunks {
        ...
        registry.db.query(sql)
            .bind(("id", chunk_id))
            .bind(("hash", format!("sha256:{}", chunk_id)))
    ...
    println!("\n🔍 Testing Text Search: 'authentication'");
    ...
    let query_embedding = registry.embedding_manager.as_ref().unwrap()
    ...
    println!("\n🔍 Testing Hybrid Search: 'user authentication' (text + semantic)");

    let hybrid_embedding = registry.embedding_manager.as_ref().unwrap()
        .get_chunk_embedding("query", "user authentication").await?;

    let hybrid_query = SearchQuery {
        text: Some("user authentication".to_string()),
